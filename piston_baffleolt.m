...
baffle_rad = 2;
baffle_z = 0;
thickness = 0.5; % changing thickness seems to have no effect since interior pb is omitted
...
for i = 1:length(fr)
    ...
    normalized_spl = spl_values - max(spl_values);
    pax = polarplot(flip(theta), flip(normalized_spl), 'LineWidth', 2, 'DisplayName', [num2str(fr(i)) ' Hz']);
    hold on;
end
grid on;
