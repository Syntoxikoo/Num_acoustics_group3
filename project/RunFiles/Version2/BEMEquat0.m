...
%
%  [A,B,C]=BEMEquat0(rzb,Topology,k,m{,chiefpoints})
%  [A,B,C]=BEMEquat0rzb,Topology,k,m,chiefpoints,filename)
%
...
A = zeros(M+nchiefp, M, length(m));
B = zeros(M+nchiefp, M, length(m));
C = zeros(M+nchiefp, 1);
if para ==true
    disp("computing matrices in parallel")

    parfor ii=1:M+nchiefp
      if ii <= M
         przb=rzb(ii,:);
      else
         przb=chiefpoints(ii-M,:);
      end
      cii=4*pi*(1+sign(przb(3)))/2;
      Am=zeros(M,length(m));
      Bm=zeros(M,length(m));
    
      for iel=1:nel
        elknrzb =rzb(Topology(iel,1:nknel),:);
        % Singular part
        [g,h,cjj] = intF2(przb,elknrzb,m);
        cii=cii+cjj;
        Am(Topology(iel,1:nknel),:)=Am(Topology(iel,1:nknel),:)+h(1:nknel,:);
        Bm(Topology(iel,1:nknel),:)=Bm(Topology(iel,1:nknel),:)+g(1:nknel,:);
        % Oscillating part
      ...
      fprintf('Row %g/%g C constant = %1.15e\n',ii,M+nchiefp,cii/4/pi);
    
      if ii <= M
         Am(ii,:)=Am(ii,:)-cii;
         C_local(ii)=cii;  % Use a temporary variable
      end
      ...
      % For file-based storage, you'll need to modify this approach
    end
    
    % If using file-based storage, you would need to handle that differently
    if ~isempty(filename)
      % Save the complete matrices instead of row by row
      save(filename, 'A', 'B', '-append');
    end
else
...
%      eval(['Bm' int2str(im) '=zeros(M+nchiefp,M);']);            % Bm$im$=zeros(M+nchiefp,M);    *********************
%      for ii=1:M+nchiefp
