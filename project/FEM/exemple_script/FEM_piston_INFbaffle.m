...
% the geometry folder.
% save("project/FEM/geometry/pistonInf_10meterFF.mat","gd","ns","sf"); 
load("project/FEM/geometry/pistonInf_10meterFF.mat")
...
bInner = applyBoundaryCondition(model,'neumann','Edge',(2),'g',pistBCFunc,'q',0); % Hard surface

% BAFFLE Surface:
bInner = applyBoundaryCondition(model,'neumann','Edge',[1,3],'g',0,'q',0); % Hard surface


% FREE FIELD: set to the characteristic impedance rho*c (non-reflecting).
outerBCFunc = @(loc,state)1j*k; 
bOuter = applyBoundaryCondition(model,'neumann','Edge',[4,5],'g',0,'q',outerBCFunc); % This is equivalent to a (rho*c) impedance
...
Node_idx = model.Mesh.findNodes("region","Edge",[4,5]);
...
polarplot(theta, Nspl, 'LineWidth', 2, 'DisplayName', [num2str(fr) ' Hz']);
% 
pax = gca;
