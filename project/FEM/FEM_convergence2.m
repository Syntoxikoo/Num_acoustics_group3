...
theta = atan2(y_true,x_true);

angles = deg2rad([33, 55, 78, 135]);
idxs = zeros(length(angles),1);
for ii= 1:length(angles)
    [~,idxs(ii)] = min(abs(theta-angles(ii)));
end
fpXY = [y_true(idxs);x_true(idxs)]';

load("FEM_groundTruthrec.mat")
F2 = scatteredInterpolant(x_true.',y_true.',p_true);

load("FEM_conv_flanged.mat")
errflanged = zeros(1, length(Mfem));
for ii = 1:length(errflanged)
    F = scatteredInterpolant(cell2mat(X(ii))',cell2mat(Y(ii))',cell2mat(p_arr(ii)));
    pFlanged_GT = F1(fpXY(:,1),fpXY(:,2));
    pFlanged = F(fpXY(:,1),fpXY(:,2));
    
    errflanged(ii) = mean(abs(pFlanged_GT- pFlanged)./abs(pFlanged_GT));
end

errRec = zeros(1, length(Mfem));
load("FEM_conv_unflanged.mat")
for ii = 1:length(errRec)
    F = scatteredInterpolant(cell2mat(X(ii))',cell2mat(Y(ii))',cell2mat(p_arr(ii)));
    pRec_GT = F2(fpXY(:,1),fpXY(:,2));
    pRec = F(fpXY(:,1),fpXY(:,2));
    
    errRec(ii) = mean(abs(pRec_GT- pRec)./abs(pRec_GT));
end


figure;
loglog(errflanged,'-o',"LineWidth",2); hold on
loglog(errRec,'-o',"LineWidth",2); 
title('Convergence of FEM (analytical is \lambda /10)' )
xlabel('Number of elements'); ylabel('Relative error')
grid

saveas(gcf, "project/figures/convergenceFEM2.svg")