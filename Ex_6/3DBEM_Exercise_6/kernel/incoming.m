...
% pI=incoming(sources,xyzb,k,PlaneON)
%
...
if PlaneON % add specular reflections of the incoming sources
    n_sources=size(srs,1);
    for ii=1:n_sources
        if isnan(srs(ii,1))
            srs=[srs ; [srs(ii,1) pi-srs(ii,2) srs(ii,3:5)]];
        else
            srs=[srs ; [srs(ii,1:2) -srs(ii,3) srs(ii,4:5)]];
        end
    end
end
