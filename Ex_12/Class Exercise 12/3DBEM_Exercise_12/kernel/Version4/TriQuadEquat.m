...
% triangular elements only. Unused velocity expnaxion code removed from the
% loop (TRI only).
...
for jj=1:N
    ...
    if geometry(jj)==1 | geometry(jj)==2 % TRI3 and TRI6
        ...
        elenodes=topotrib(jj,1:nknel_tri);
        elknxyzb=xyzb(elenodes,:);
        Aele=zeros(M,nknel_tri);Bele=zeros(M,nknel_tri); % Buffer matrices to increase speed
        ...
        for ikn=1:nknel_tri
            ...
            CConst(elenodes(ikn))=CConst(elenodes(ikn))+CK12;
            Aele(elenodes(ikn),:)=Aele(elenodes(ikn),:)+F12A;
            Bele(elenodes(ikn),:)=Bele(elenodes(ikn),:)+F12B;
        end
        ...
        for ii=1:M
            %disp(sprintf('Node %g element %g',ii,jj));
            if isempty(find(elenodes==ii, 1)) % If ii is not among elenodes
               if nsingON==1
                  is_close=nsingcheck(xyzb(ii,:),elknxyzb);
               else
                  is_close=0;
