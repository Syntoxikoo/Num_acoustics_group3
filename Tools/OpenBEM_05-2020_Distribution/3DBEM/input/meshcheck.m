...
% [nodesb,elementsb,segments]=meshcheck(nodes,elements,faulty,plotgeom)
%
...
%                If consecutive nodes are repeated, one is removed.
...
%                is connected to more than two elements, column 4 is set to
...
% Vicente Cutanda Henríquez, 5-2011, version 11-2011

% Vicente Cutanda Henríquez, 6-2013: quadratic (triangular and
% quadrilateral) elements introduced
...
% Remove consecutive repeated nodes and detect number of nodes in every
% element. Fill with NaN at the end and generate "elsizes", a column vector
% containing the number of nodes in each element.
elsizes=ones(N,1)*ncol;
for ee=1:N
    for nn=1:ncol % Remove repeated consecutive nodes
        n1=elements(ee,nn);n2=elements(ee,mod(nn,ncol)+1); % nodes in the current segment
        if n1==n2 & ~isnan(n1)
            elements(ee,mod(nn,ncol)+1)=NaN;
        end
    end
    if any(isnan(elements(ee,:))) % place NaNs at the end and change "elsizes" if needed
        elements(ee,:)=[elements(ee,~isnan(elements(ee,:))) elements(ee,isnan(elements(ee,:)))];
        elsizes(ee,1)=length(elements(ee,~isnan(elements(ee,:))));
    end
end
elements=elements(:,1:max(elsizes)); % Remove columns with only NaNs
elements=[elements ones(N,1)*NaN];   % Include a new column of NaNs, to be filled with body numbers
...
for bb=1:body
   ...
   if dot(cross(farnod(3,:)-farnod(2,:),farnod(1,:)-farnod(2,:)),cent_b-farnod(2,:))>0
       ...
       for jj=1:Nb
           nnod=elsizes(tx(jj));
           if nnod==6 | nnod==8 % quadratic triangular or cuadrilateral element
               nnod=nnod/2;
               elements(tx(jj),1:nnod)=elements(tx(jj),nnod:-1:1);
               elements(tx(jj),nnod+1:nnod*2)=elements(tx(jj),mod((nnod:-1:1)-2,nnod)+1+nnod);
           else
               elements(tx(jj),1:nnod)=elements(tx(jj),nnod:-1:1);
           end
       end
   ...
   if plotgeom==1
       ...
       for jj=1:Nb % draw elements in the current body one by one
           ...
           if nnod==6 | nnod==8;
               ...
               hold on
               plot3(nodes(elements(tx(jj),nnod+1:2*nnod),1),nodes(elements(tx(jj),nnod+1:2*nnod),2),nodes(elements(tx(jj),nnod+1:2*nnod),3),'k.');
           else
               patch('faces',1:nnod,'vertices',nodes(elements(tx(jj),1:nnod),1:3),'FaceVertexCData',ones(nnod,1)*Cdata(tx(jj),:),'FaceColor','flat');
               hold on
           end
...
if any(isnan(segments(:,4))) || any(isinf(segments(:,4)))
    disp('Errors were detected in the mesh. It may not be suitable for BEM calculations.')
    if faulty
        nodesb=nodes;
        elementsb=elements;
    else
...
% been analyzed.
% QUAD version: treats quadratic elements, where segments have three nodes
% instead of two.
%
%
...
%         end
    ...
    for ee=1:nnod % examine all segments in the element
        ...
        if ~isempty(segments)
            inlist=find(((segments(:,1)==n1 & segments(:,2)==n2) | (segments(:,1)==n2 & segments(:,2)==n1)) & segments(:,5)==body); %<<<<<<<<<<<<<<<<<<<
        else
            inlist=[];
        end
        ...
        elseif isempty(inlist) % process a new entry of "segments"
            %       elnr_new=[];segnr_new=[];
            [ff,cc]=find(elements(:,1:nnod)==n1); % find new elements with the same segment
            ffel=find(ff==elnr);
            ...
            if ne>1
                ...
                if QUAD, nodes(elements(elnr,mod(ee-1,nnod)+1+nnod),end)=body; end
                if nargin>7
            ...
            elseif ne==0
                ...
                % Check the central node in the segment, if it is quadratic
                if QUAD 
                    if chg_sgn && (elements(elnr,mod(ee-1,nnod)+1+nnod) ~= elements(ff2,mod(cc2-1,nnod)+1+nnod)) | ...
                      ~chg_sgn && (elements(elnr,mod(ee-1,nnod)+1+nnod) ~= elements(ff2,mod(cc2-2,nnod)+1+nnod))  
                        error(['Meshcheck error. Central node in a quadratic segment mismatch, element: ' ...
                                num2str(elnr) ', nodes: ' num2str(n1) ', ' num2str(n2)]);
                    end
                    nodes(elements(elnr,mod(ee-1,nnod)+1+nnod),end)=body;
                end
                ...
                if any(ele_stack==ff2)
                    ...
                    if chg_sgn % invert order of the nodes if needed
                        ...
                        if QUAD
                            elements(ff2,nnod+1:nnod*2)=elements(ff2,mod((nnod:-1:1)-2,nnod)+1+nnod);
                        end
                    end
                end
            end
