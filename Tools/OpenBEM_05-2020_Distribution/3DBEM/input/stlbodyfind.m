 function [nodesb,topologyb,toposhrinkb,tim,segmopen]=stlbodyfind(nodes,topology)
...
while ~isempty(find(topology(:,end)==0)) % body loop
   ...
   while ~isempty(segms) % segment loop
      ...
      [segmout,topology,err_code]=elstep(topology,segmin,body,nknel,N,isquad);
      if err_code==1
          if showprogress
             patch('faces',topology(segmin(1),1:3),'vertices',nodes(:,1:3),'FaceColor','r');
             for iii=1:size(segmout,1)
                 patch('faces',topology(segmout(iii,1),1:3),'vertices',nodes(:,1:3),'FaceColor','y');
             end
...
%               num2str(segmout(1))]);
         ...
         if topology(segmout(1),end)==0 % unchecked element found
            ...
            % added aug 2008 warning: only triangular elements - not
            % general
            if showprogress
                %patch('faces',topology(segmout(1),1:3),'vertices',nodes(:,1:3),'FaceVertexCData',[0.1 0.1 0.1],'FaceColor','flat');
                patch('faces',topology(segmout(1),1:3),'vertices',nodes(:,1:3),'FaceColor','g'...
                    ,'FaceAlpha', 0.1, 'EdgeAlpha',0.1);
            end
            % finished add aug 2008
...
function [segmout,topology,err_code]=elstep(topology,segmin,body,nknel,N,isquad)
...
%    -err_code: 0 if no errors found - otherwise 1 (adjoining elements)
...
if (szgood+szbad)>1
   if topology(segmin(1),mod((segmin(2)-1),nknel)+1)==topology(segmin(1),mod(segmin(2),nknel)+1)
      segmout=[1 0];
       ...
       disp(sprintf('In elstep at element %g , segment %g',segmin(1),segmin(2)));
       disp('Warning: more than two adjoining elements - Mesh invalid');
       err_code=1;
       segmout=[segood; segbad];
       return;
   end
