function [nodesb,topologyb,toposhrinkb,tim,segmopen]=bodyfind(nodes,topology)

% [nodesb,topologyb,toposhrinkb,tim,segmopen]=bodyfind(nodes,topology)
%
...
% Modified to admit triangular elements. VCH 02-2011

% Revision: The normal vectors are flipped together if they point in the
% wrong direction (line 170), but this was not done correctly for quadratic
% quadrilateral elements. This has been corrected. 05-2007 VCH
...
while ~isempty(find(topology(:,end)==0)) % body loop
   ...
   while ~isempty(segms) % segment loop
      ...
      [segmout,topology]=elstep(topology,segmin,body,nknel,N,quad);
...
%               num2str(segmout(1))]);
         ...
         if topology(segmout(1),end)==0 % unchecked element found
            ...
            nodes(topology(segmout(1),1:nknel*(1+quad)),4)=body;
...
for bb=1:body
   ...
   topologytmp=topology(tx,1:nknel*(quad+1));
...
%   Cdata=[ones(Nb,1)]*[1 0 0]; % color data, spiderman version
   ...
   for ff=1:Nb  % Find the position and the normal vector at the center of body's elements
      ...
      if size(elknxyzb,1)==3
      [psi, xq, yq, zq, nx, ny, nz]=elemshapetri([elknxyzb ones(nknel,1)],[1/3 1/3 1/3]); % Select point in center
      else
      [psi, xq, yq, zq, nx, ny, nz]=elemshape([elknxyzb ones(nknel*(quad+1),1)],[0 0 0 1]); % Select point in center
      end
   ...
   if direction < 0
      if quad
         topology(tx,:)=topology(tx,[4 3 2 1 7 6 5 8 9]);
      else
...
function [segmout,topology]=elstep(topology,segmin,body,nknel,N,quad)
...
if (szgood+szbad)>1
   if topology(segmin(1),mod((segmin(2)-1),nknel)+1)==topology(segmin(1),mod(segmin(2),nknel)+1)
      ...
      error('Error: more than two adjoining elements - Checking aborted');
   end
...
elseif szbad==1
   ...
   topology(segbad(1),col(3))=dummy(col(4));
