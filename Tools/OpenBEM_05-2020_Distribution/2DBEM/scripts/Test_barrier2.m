...
sigmaB=[Inf Inf Inf Inf]; % Reflecting barrier  <<<<< UPDATE IS MORE SEGMEnTS ARE INTRODUCED IN THE GEOMETRY 
...
bar=[-Bwdt/2 0;-Bwdt/2 Bhgt;Bwdt/2 Bhgt;Bwdt/2 0];
% barrier top
topsegs=[bar(2,:) bar(3,:) ceil(sqrt(sum((bar(2,:)-bar(3,:)).^2))*el_wl) 0 0];
%topsegs=[bar(2,:) bar(3,:) 20 0.5*Bwdt 0];
...
% define geometry
segmentsB=[bar(1,:) bar(2,:) ceil(sqrt(sum((bar(1,:)-bar(2,:)).^2))*el_wl) 0 0;...
%    bar(2,:) bar(3,:) ceil(sqrt(sum((bar(2,:)-bar(3,:)).^2))*el_wl) 0 0;...
    topsegs;
    bar(3,:) bar(4,:) ceil(sqrt(sum((bar(3,:)-bar(4,:)).^2))*el_wl) 0 0;...
    bar(4,:) bar(1,:) ceil(sqrt(sum((bar(4,:)-bar(1,:)).^2))*el_wl) 0 0];
