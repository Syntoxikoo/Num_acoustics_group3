% Half sphere on a plane, with a plane wave coming from an angle
...
segments=[0 R R 0 20 R el_wl; R 0 0 0 15 0 el_wl];
...
ptotANA=2*PlaneWaveScatSphere(kp,R,R,linspace(AngleS+pi/2,AngleS+3*pi/2,100),1e-6); % Twice due to plane

% Figure: pressure modulus over the surface, on a plane at angle "phi_angle" from the rho-z plane
figure;
ps_fb=[ps_back(end:-1:2);ps_front];
plot(linspace(0,360,2*Mnd-1),abs(ps_fb),'-x',linspace(0,360,length(ptotANA)),abs(ptotANA),'-');grid
...
legend('BEM','Analytical','Location','southeast');
...
Nzz=20; zz=linspace(0,2*R,Nzz);%linspace(-Dh-Lh,Dh+Lh,Nzz);
