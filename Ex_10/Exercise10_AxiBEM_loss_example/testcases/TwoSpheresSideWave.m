% Two spheres with a plane wave coming from an angle
...
D=3*R;                  % Distance between centres
A0=1;                   % Wave amplitude
...
AngleS=90*(pi/180);    % Angle of incidence of a plane wave in the rho-z plane, radians
...
segments=[0 R+D/2 R D/2 20 R el_wl; R D/2 0 D/2-R 20 R el_wl; ...
          0 R-D/2 R -D/2 20 R el_wl; R -D/2 0 -D/2-R 20 R el_wl];
...
rzb_chief=[linspace(R*0.1,R*0.6,5)' linspace(R*0.11,-R*0.5,5)'+D/2 ones(5,1); ...
           linspace(R*0.1,R*0.6,5)' linspace(R*0.11,-R*0.5,5)'-D/2 ones(5,1)];
...
figure;
ps_fb1=[ps_front(1:end/2-1); ps_back(end/2:-1:1)];
ps_fb2=[ps_front(end:-1:end/2+2); ps_back(end/2+1:end)];
plot(linspace(0,360,M-1),abs(ps_fb1),'-x',linspace(0,360,M-1),abs(ps_fb2),'-o');grid
...
legend('BEM sphere z>0','BEM sphere z<0 (inverted)','Location','southwest');
...
Nzz=20; zz=linspace(-5*R,5*R,Nzz);%linspace(-Dh-Lh,Dh+Lh,Nzz);
