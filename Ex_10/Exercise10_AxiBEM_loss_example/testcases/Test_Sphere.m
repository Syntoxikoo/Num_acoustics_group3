...
[pfAN, v_r, v_theta] = SphereFirstOrder(k,c,rho,R,u0,[R*ones(M,1) acos(rzb(:,2)/R)]);
...
xlabel('Angle');ylabel('|pressure on field points|');
legend('Analytical','BEM')
title(['First-order oscillating sphere, k=' num2str(k) ', Rfp=' num2str(Rfp/R) '*R m']);
subplot(3,1,2)
plot(acos(rzb(:,2)/R)*180/pi,abs(pfAN),'-ko',acos(rzb(:,2)/R)*180/pi,abs(pf),'-rx'); grid
xlabel('Angle');ylabel('|pressure on the surface|');
legend('Analytical','BEM')
subplot(3,1,3)
plot(acos(rzb(:,2)/R)*180/pi,abs(v_r),'-gd',acos(rzb(:,2)/R)*180/pi,abs(v_theta),'-k^'); grid
xlabel('Angle');ylabel('|analytical velocity on the surface|');
